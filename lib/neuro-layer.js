...
var NeuroLayer = function (config) {
	...
	this.randomInit = function (nCols, nCells, nSegs, nSyns) {
		...
		for (; i< nCols*nCells; i++) {
			this.addCell(i, Math.floor(i/nCells), true);
		}
	...
	this.getMatrix = function () {
		...
		var matrix = segments.map(function (seg) {
			...
			colHeader.push([listeningCol, listeningCell, seg])
			return row;
		});
	...
	this.getMatrix2 = function () {
		...
		var matrix = segmentsByCells.map(function (cellSegs) {
			...
			return [cell, segments.map(function (seg) {
				var listeningCell = that.getListeningCell(seg);
				var listeningCol = 
						listeningCell !== null ? 
							that.getColumn(listeningCell) :  
	...
	this.inspect = function (depth) {
		...
		output += Array(4).join(" ") + "    " + rowHeader.map(fh.snd).map(pad).join(" , ");
		output += "\n\n"
		output += matrix.data.map(function (row) {
			var cell = row[0];
			var cons = row[1];
			var line = "";
			// line += pad(cell);
			line += cons.map(function (row) {
				return pad(cell) + " <- " + row.map(pad).join(" , ");
			}).join('\n');
			return line
		}).join('\n');
