...
TemporalPooler.prototype.processInput = function (input) {
	...
	// activity
	// @todo last prediction should be filtered, 
	// 		it should only contain sequence segments
	// @todo check back with the white paper, and see if we can speed that up, 
	// 		or do it on the run somewhere else
...
TemporalPooler.prototype.suggestLearningUpdates = function (col, t) {
	...
	// to active score ...
	// ==========================
	var bestSeg = max(segments, activeScore);
...
TemporalPooler.prototype.learn = function () {
	...
	// Learning I.
	var suggestions = input.map(function (col) {
		return that.suggestLearningUpdates(col);
	}).reduce(concat, []);
